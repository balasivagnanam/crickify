...
getBestBatting() {
  ...
  getBowlingSummary(bowlingId) {
    return new Promise((resolve, reject) => {
      let headers = new Headers();
    headers.set('Content-Type', 'application/json');
    headers.set("token",this.getToken());
      this.http.get(bowlingapiUrl + '/bowling/' + bowlingId, {headers: headers})
        .subscribe(res => {
          resolve(res.json());
          console.log("auth response matches", res.json()); 
          if(res.json().statusCode == '200'){
 ...
 addBowling(data,match) {
    return new Promise((resolve, reject) => {
      let headers = new Headers();
    headers.set('Content-Type', 'application/json');
    headers.set("token",this.getToken());
	console.log("requeset", JSON.stringify(data)); 
      this.http.post(bowlingapiUrl + '/bowling/'+match, JSON.stringify(data), {headers: headers})
        .subscribe(res => {
          resolve(res.json());
